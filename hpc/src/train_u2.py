...
# criterion = None (is set to Multi_Fusion_Loss inside)
...
if __name__ == "__main__":
    tload, vload = getDataloaders('/zhome/4e/8/181483/deep-learning-project/data/final_dataset.npy', batch_size=6)
    model = make_u2net()                                # <<< CHANGE MODEL HERE!! make_uresnest(enc_layers="50"/"101"/"200"),
                                                          # make_unet() or make_u2net()
    criterion = None                              # <<< CHANGE LOSS HERE!! nn.CrossEntropyLoss(), nn.BCELoss() or None
    optimizer = optim.Adam(model.parameters())    # <<< CHANGE OPTIMIZER HERE!! optim.Adam() or optim.SGD()
    epochs = 101                                          # <<< CHANGE EPOCHS HERE!!
