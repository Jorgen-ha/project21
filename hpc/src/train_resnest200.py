...
def train(model, train_dataloader, val_dataloader, optimizer, criterion=None, epo_num=10):
    """Training loop for three models: ResNeSt, UNet, U2Net
    ...
    for epo in trange(epo_num):
        ...
        if np.mod(epo, 10) == 0:
            ...
            # save model
            filename = f'{model_name}_{epo+220}_loss_trian_{round(train_loss/len(train_dataloader),5)}_val_{round(val_loss/len(val_dataloader),5)}.pt'
...
if __name__ == "__main__":
    tload, vload = getDataloaders('/zhome/4e/8/181483/deep-learning-project/data/final_dataset.npy', batch_size=3)
    #model = make_uresnet(layers="200", drop=0.7)           # <<< CHANGE MODEL HERE!! make_uresnest(layers="50"/"101"/"200", drop=0.0->0.9999...)
                                                          # make_unet() or make_u2net()
    model = torch.load("/zhome/4e/8/181483/deep-learning-project/ResNeSt200_220_loss_trian_0.00927_val_0.00951.pt")
    criterion = nn.BCELoss()                              # <<< CHANGE LOSS HERE!! nn.CrossEntropyLoss() or nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-4)   # <<< CHANGE OPTIMIZER HERE!! optim.Adam() or optim.SGD()
    epochs = 31                                            # <<< CHANGE EPOCHS HERE!!
