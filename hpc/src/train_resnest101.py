...
if __name__ == "__main__":
    ...
    #model = make_uresnet(layers="101", drop=0.7)           # <<< CHANGE MODEL HERE!! make_uresnest(layers="50"/"101"/"200", drop=0.0->0.9999...)
                                                          # make_unet() or make_u2net()
    model = torch.load("/zhome/4e/8/181483/deep-learning-project/ResNeSt101_220_loss_trian_0.00749_val_0.00929.pt")
    criterion = nn.BCELoss()                              # <<< CHANGE LOSS HERE!! nn.CrossEntropyLoss() or nn.BCELoss()
    #optimizer = optim.SGD(model.parameters(), lr=1e-2, momentum=0.4)
    optimizer = optim.Adam(model.parameters(), lr=1e-4)   # <<< CHANGE OPTIMIZER HERE!! optim.Adam() or optim.SGD()
    epochs = 201                                            # <<< CHANGE EPOCHS HERE!!
