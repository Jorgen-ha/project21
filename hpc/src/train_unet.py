...
############################ NOTES ###############
# Defaults from paper:
# criterion = BCELoss() (they used normal crossEntropy)
# optimizer = optim.SGD(lr=e-2, mom=0.99)
# batch size = 1
# epochs = ?
...
if __name__ == "__main__":
    tload, vload = getDataloaders('/zhome/4e/8/181483/deep-learning-project/data/final_dataset.npy', batch_size=10)
    model = make_unet()                                # <<< CHANGE MODEL HERE!! make_uresnest(enc_layers="50"/"101"/"200"),
                                                          # make_unet() or make_u2net()
    criterion = nn.BCELoss()                              # <<< CHANGE LOSS HERE!! nn.CrossEntropyLoss() or nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    #optimizer = optim.SGD(model.parameters(), lr=1e-3, momentum=0.7) # <<< CHANGE OPTIMIZER HERE!! optim.Adam() or optim.SGD()
